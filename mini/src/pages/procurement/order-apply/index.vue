              ...
              <div class="goods-item" v-for="(val,i) in item.goods" :key="val.id">
                ...
                <div class="goods-infor">
                  ...
                  <div class="goods-unit_price">
                    <div>单价：</div>
                    <input type="digit" v-model="val.unit_price" @change="unit_priceChange">
                    <div>元/{{val.unit}}</div>
                  </div>
                </div>
        ...
        <div v-for="(item,index) in data" :key="item.el_id" v-if="item.goods.length>0">
          <div class="order-list-item" v-for="(val,i) in item.goods" :key="i" v-if="val.amount>0">
            ...
            <div class="text_center">
              <input style="line-height: 40px;height: 40px;" type="digit" v-model="val.unit_price" placeholder="0" @change="unit_priceChange">
            </div>
                  ...
                  if(val.goods.length>0){
                    for(let v of val.goods){
                      if(v.id === item.goods_id){
                        ...
                        v.unit_price = item.unit_price
                      }
                    }
                  }
                }
              }
              _this.totalAmountChange()
            })
          }
        })
      },
      ...
      totalAmountChange(){
        ...
        for(let item of this.data){
          for(let val of item.goods){
            if(val.amount > 0){
              goodsCount++
              total_amount+=Number(val.amount)*Number(val.unit_price)
            }
          }
        }
